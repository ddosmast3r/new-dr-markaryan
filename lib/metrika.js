...
// Opt-in: счётчик стартует только после явного «Принять» в cookie-баннере.
// Выбор хранится в localStorage, поэтому переживает перезагрузку страницы.
...
export function initMetrika() {
  if (!YM_ID || !hasWindow() || !hasAnalyticsConsent()) return false;
...
export function reachGoal(goal) {
  if (!YM_ID || !hasWindow() || !hasAnalyticsConsent()) return;
