...
// Opt-out: аналитика работает по умолчанию и выключается только явным отказом.
// Отказ хранится в localStorage, поэтому переживает перезагрузку страницы.
function hasConsent() {
  if (!hasWindow()) return false;
  try {
    return window.localStorage.getItem(COOKIE_CONSENT_KEY) !== 'declined';
  } catch {
    return false;
  }
}

// Удаляем cookie, которые успел поставить счётчик до отказа.
export function clearMetrikaCookies() {
  if (!hasWindow()) return;
  const domains = [location.hostname, `.${location.hostname}`];
  document.cookie.split(';').forEach((entry) => {
    const name = entry.split('=')[0].trim();
    if (!name.startsWith('_ym') && name !== '_yasessionid') return;
...
export function initMetrika() {
  if (!YM_ID || !hasWindow() || !hasConsent()) return false;
